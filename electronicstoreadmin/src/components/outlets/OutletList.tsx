...
const OutletList = () => {
    ...
    } = usePagedApiRequest<OutletResponse, []>(
        fetchOutletsApi,
        true
    );
    ...
    const handleDeleteOutlet = async () => {
        ...
        try {
            ...
            showNotification(`Error deleting outlet: ${error instanceof Error ? error?.message : 'Unknown error'}`, 'error');
        }
    };
    ...
    const handleToggleStatus = async (outlet: OutletResponse) => {
        try {
            ...
            console.log(`Toggling outlet ${outlet?.id} status from ${outlet?.active} to ${!outlet?.active}`);

            const result = await OutletService.updateOutlet(outlet?.id, updatedOutlet);

            if (result && result?.status === 'SUCCESS') {
                showNotification(`Outlet ${!outlet?.active ? 'activated' : 'deactivated'} successfully`, 'success');
                // Refetch outlets to ensure our state is synced with the backend
                fetchOutlets();
            } else {
                showNotification(`Failed to update outlet status: ${result?.message || 'Unknown error'}`, 'error');
            }
        } catch (error) {
    ...
    const columns = [
        ...
        },
        {
            ...
            render: (item: OutletResponse) => (
                <Box>
                    <Typography variant="body2">{item?.locality?.name}, {item?.locality?.pincode}</Typography>
                    <Typography variant="caption" color="textSecondary">{item?.city?.name}, {item?.state?.name}</Typography>
                </Box>
            ),
        },
        {
    ...
    const actions: Action<OutletResponse>[] = [
        {
            icon: (item: OutletResponse) => <EditIcon />,
            label: 'Edit Outlet',
        ...
        {
            icon: (item: OutletResponse) => (
                item?.active ? <ToggleOnIcon /> : <ToggleOffIcon />
            ),
            label: 'Toggle Status',
            onClick: handleToggleStatus,
            color: (item: OutletResponse) => (
                item?.active ? 'success' : 'default'
            ),
        },
        {
            icon: (item: OutletResponse) => <DeleteIcon />,
            label: 'Delete Outlet',
            onClick: (item: OutletResponse) => {
